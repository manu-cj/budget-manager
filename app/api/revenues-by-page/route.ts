...
import { getRevenuesByOffset  } from '@/app/controllers/incomeController';
...
export async function GET(request: Request) {
    try {
      ...
      const userId = decoded.id;
        ...
        // Appeler la fonction pour récupérer les revenues avec pagination
        const revenues = await getRevenuesByOffset(userId, limit, offset);
  
  
      return new NextResponse(JSON.stringify({expense: revenues}), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
